...
@ThreadSafe
public abstract class MyAbstractDataProviderStrategy implements MyDataProviderStrategy {

	// ------------------->> Constants

	/** Application logger */
	private static final Logger LOG = LoggerFactory.getLogger(MyAbstractDataProviderStrategy.class);
	...
	private static final int DEFAULT_NBR_COLLECTION_ELEMENTS=2;
	private Object replacedVal;
	 ...
	 */
	public MyAbstractDataProviderStrategy() {
		this(DEFAULT_NBR_COLLECTION_ELEMENTS);
	}

	public MyAbstractDataProviderStrategy(int nbrOfCollectionElements) {
		...
		while (!types.isEmpty()) {
			...
			if (null != manufacturer) {
				try {
					...
					T tmp = (T) manufacturer.getType(this, attributeMetadata,genericTypesArgumentsMap);
					tmp=replaceWithManualValues(type,tmp);
					type.getClass();
					if (null != tmp) {
	...
	private <T> T replaceWithManualValues(Class<?> type, T tmp) {
		replacedVal=tmp;
	 ...
	 */
	@Override
	public <T> MyAbstractDataProviderStrategy addOrReplaceFactory(
	 ...
	 */
	@Override
	public <T> MyAbstractDataProviderStrategy removeFactory(
	 ...
	 */
	@Override
	public MyDataProviderStrategy addOrReplaceAttributeStrategy(
	 ...
	 */
	@Override
	public MyDataProviderStrategy removeAttributeStrategy(
			final Class<? extends Annotation> annotationClass) {
